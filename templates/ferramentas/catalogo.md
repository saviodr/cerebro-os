...
**⚠️ Atenção (achado em 12/08/2026, nesta conta):** só o **primeiro** deploy de cada projeto sobe normal; todo deploy **seguinte** no mesmo projeto trava em "building" pra sempre. Publicar de novo = criar projeto com nome novo (`vercel --prod --yes --name outro-nome`). Além disso, todo projeto novo nasce com `ssoProtection`/`attackMode` ligados e devolve "Vercel Security Checkpoint" em vez do site — `curl` retorna 200 mesmo bloqueado, então **testar sempre com navegador de verdade** (Playwright), nunca só com `curl`. Pra desligar a proteção via API:
```bash
T=$(python -c "import json;print(json.load(open(r'C:\Users\<usuario>\AppData\Roaming/xdg.data/com.vercel.cli/auth.json')).get('token',''))")
curl -s -X PATCH -H "Authorization: Bearer $T" -H "Content-Type: application/json" \
  -d '{"ssoProtection":null,"passwordProtection":null}' \
  "https://api.vercel.com/v9/projects/NOME-DO-PROJETO?teamId=SEU_TEAM_ID"
```
Não investigado a fundo ainda (é característica da conta, não do código do projeto). Se resolver a causa raiz, apagar esta nota.
